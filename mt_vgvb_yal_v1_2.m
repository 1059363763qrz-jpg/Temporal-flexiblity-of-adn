function mt_vgvb_yal_v1_2()
% MT_VGVB_YAL_V1_2 First-stage VG/VB aggregate flexibility reproduction (MATLAB/YALMIP/Gurobi).
%
...
%
% Run in MATLAB: mt_vgvb_yal_v1_2
% Outputs: results_mt_vgvb_yal_v1_2/
...
outdir = 'results_mt_vgvb_yal_v1_2';
...
if config.SELF_TEST_ONLY, run_self_test(config,outdir); return; end
caseData = build_case_ieee33(config); profiles = build_profiles(config.T); resources = build_resources(config); preRows=validate_dimensions(caseData,resources,config); if ~all(cell2mat(preRows(:,2))), error('Pre-optimization dimension check failed'); end
...
config.V_MIN=0.95^2; config.V_MAX=1.05^2; config.V0=1.0^2; config.NETWORK_DECOUPLING_MODE='capacity_weighted'; config.NETWORK_DECOUPLING_MODE='capacity_weighted'; config.alpha_net=0.5; config.ALLOW_DG_Q_SUPPORT=false; config.RPC_IMPLEMENTED=false; config.RPC_PLACEHOLDER_BUSES=[18 33]; config.OLTC_OPTIMIZED=false; config.OLTC_FIXED_TAP=1.0; config.RPC_NOTE='RPC data are recorded as benchmark information only, not optimized in v1_2.'; config.OLTC_NOTE='OLTC is fixed at tap 1.0 and not optimized in v1_2.'; config.ESS_SIMULTANEOUS_MODE='loss_penalty'; config.EPS_ESS_LOSS=1e-6; config.STAGE_I_PQ_RANGE_IMPLEMENTED=false; config.STAGE_II_ACTIVE_TRAJECTORY_IMPLEMENTED=true; config.assumption_line_smax_mva=3.5;
config.OutputFlag=0; config.TimeLimit=120; config.FeasibilityTol=1e-7; config.n_random_checks=12; config.n_sensitivity_validation=8; config.SELF_TEST_ONLY=false; config.random_seed=7; config.ess_soc_assumption='ESS SOC parameters are completed by assumptions for flexibility aggregation experiments.'; config.decoupling_note='The current decoupling is a conservative margin allocation heuristic satisfying h_G_dec + h_B_dec <= h_net, not the full robust optimization decomposition in Wang & Wu (2021).';
...
function rows=validate_dimensions(c,r,config,HG,HB,h,meta,hG,hB)
rows={}; add('load_vector_length',numel(c.p_load_base)==c.nbus && numel(c.q_load_base)==c.nbus,sprintf('nbus=%d',c.nbus)); add('branch_matrix_shape',all(size(c.branches)==[c.nbranch 5]),mat2str(size(c.branches))); add('gder_bus_indices_valid',all(arrayfun(@(x)x.bus>=1 && x.bus<=c.nbus,r.G)),'G buses'); add('bder_bus_indices_valid',all(arrayfun(@(x)x.bus>=1 && x.bus<=c.nbus,r.B)),'B buses'); add('positive_branch_rx',all(c.branches(:,3)>0) && all(c.branches(:,4)>0),'r/x > 0'); add('positive_time_and_base',config.T>0 && config.DT>0 && config.BASE_MVA>0 && config.BASE_KV>0,sprintf('T=%g DT=%g',config.T,config.DT));
if nargin>=4, add('network_row_counts',size(HG,1)==size(HB,1) && size(HG,1)==numel(h) && size(HG,1)==numel(meta),sprintf('HG=%s HB=%s',mat2str(size(HG)),mat2str(size(HB)))); add('H_G_column_count',size(HG,2)==2*numel(r.G)*config.T,sprintf('actual=%d expected=%d',size(HG,2),2*numel(r.G)*config.T)); add('H_B_column_count',size(HB,2)==numel(r.B)*config.T,sprintf('actual=%d expected=%d',size(HB,2),numel(r.B)*config.T)); add('finite_network_arrays',all(isfinite(HG(:))) && all(isfinite(HB(:))) && all(isfinite(h(:))),'finite H/H/h'); end
if nargin>=8, add('decoupled_margins_valid',all(hG+hB<=h+1e-8),sprintf('max_violation=%g',max(hG+hB-h))); end
    function add(name,ok,detail), rows(end+1,:)={name,logical(ok),detail}; end
end

function run_self_test(config,outdir)
fprintf('[SELF-TEST] Running data/dimension checks without Gurobi...\n');
c=build_case_ieee33(config); p=build_profiles(config.T); r=build_resources(config); [D,A]=topo_matrices(c); rows=validate_dimensions(c,r,config); rows=[rows; {'total_P_load_check',abs(c.total_P_load-3.715)<=1e-6,c.total_P_load}; {'total_Q_load_check',abs(c.total_Q_load-2.300)<=1e-6,c.total_Q_load}; {'downstream_matrix_shape',all(size(D)==[c.nbranch c.nbus]),mat2str(size(D))}; {'path_matrix_shape',all(size(A)==[c.nbus c.nbranch]),mat2str(size(A))}]; writecell([{'check','passed','detail'};rows],fullfile(outdir,'self_test_summary.csv')); writecell([{'field','value'}; case_parameter_summary(c,r,config)],fullfile(outdir,'case_parameter_summary.csv')); writematrix([(1:c.nbus)',c.p_load_base,c.q_load_base],fullfile(outdir,'bus_load_table_used.csv')); fprintf('[SELF-TEST] Done. No Gurobi calls were made.\n');
end
...
rows={'CASE_PARAM_MODE',config.CASE_PARAM_MODE;'BASE_KV',config.BASE_KV;'BASE_MVA',config.BASE_MVA;'Z_BASE',config.Z_BASE;'total_P_load',c.total_P_load;'total_Q_load',c.total_Q_load;'bus17_PQ',sprintf('P=%g,Q=%g',c.p_load_base(17),c.q_load_base(17));'bus18_PQ',sprintf('P=%g,Q=%g',c.p_load_base(18),c.q_load_base(18));'DG buses/capacity',sprintf('18/22/25/33: 0.2 MW each, q default zero');'ESS buses/power/energy',sprintf('18/22/25/33: 0.1 MW / 0.2 MWh each');'ESS SOC assumption',config.ess_soc_assumption;'RPC status',sprintf('%s placeholder buses [%d %d]',config.RPC_NOTE,config.RPC_PLACEHOLDER_BUSES(1),config.RPC_PLACEHOLDER_BUSES(2));'OLTC status',config.OLTC_NOTE;'line rating assumption',sprintf('Smax=%g MVA where Table IV is not provided',config.assumption_line_smax_mva);'stage_i_pq_range_implemented',config.STAGE_I_PQ_RANGE_IMPLEMENTED;'stage_ii_active_trajectory_implemented',config.STAGE_II_ACTIVE_TRAJECTORY_IMPLEMENTED;'decoupling_note',config.decoupling_note};
end

function export_results(outdir,config,data,bg,bb,AG,bG,AB,bB,randG,randB,chkG,chkB,sensRows)
fprintf('[6/7] Exporting CSV/MAT results...\n'); writematrix([(1:config.T)',data.base.p0(:),data.base.q0(:)],fullfile(outdir,'base_dispatch.csv')); writecell([{'field','value'}; case_parameter_summary(data.caseData,data.resources,config)],fullfile(outdir,'case_parameter_summary.csv')); writematrix([(1:data.caseData.nbus)',data.caseData.p_load_base,data.caseData.q_load_base],fullfile(outdir,'bus_load_table_used.csv')); writecell([{'check','passed','detail'}; validate_dimensions(data.caseData,data.resources,config)],fullfile(outdir,'self_test_summary.csv'));
...
writecell([{'row','type','time','index','base_value','h_net'};rows],fullfile(outdir,'network_margins.csv')); writecell([{'row','type','time','index','h_net','h_G_dec','h_B_dec','range_G','range_B'};dec],fullfile(outdir,'decoupling_summary.csv')); writecell([{'constraint_type','max_abs_error','mean_abs_error','n_tests'};sensRows],fullfile(outdir,'sensitivity_validation.csv')); writecell([{'check','passed','detail'}; validate_dimensions(data.caseData,data.resources,config,data.H_G,data.H_B,data.h_net,data.row_meta,data.h_G_dec,data.h_B_dec)],fullfile(outdir,'dimension_check_summary.csv')); writecell({'check','value';'h_G_dec_plus_h_B_dec_le_h_net',all(data.h_G_dec+data.h_B_dec<=data.h_net+1e-8);'max_violation',max(data.h_G_dec+data.h_B_dec-data.h_net);'note',config.decoupling_note},fullfile(outdir,'decoupling_validity_check.csv'));
...
bglog=bg.log; writecell([{'problem','status','true_objective','penalized_objective','runtime_sec','simultaneous_ch_dis_max'}; bglog; bb.log],fullfile(outdir,'solver_log.csv')); essRows=bb.log(:,[1 6]); writecell([{'solution','simultaneous_ch_dis_max'}; essRows],fullfile(outdir,'ess_simultaneous_diagnostic.csv')); writematrix([(1:size(chkG,1))',chkG],fullfile(outdir,'no_shrink_feasibility_check_g.csv')); writematrix([(1:size(chkB,1))',chkB],fullfile(outdir,'no_shrink_feasibility_check_b.csv'));

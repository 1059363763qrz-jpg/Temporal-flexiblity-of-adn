...
# -*- coding: utf-8 -*-
"""
First-stage VG/VB aggregate flexibility reproduction v1_1 (Python/Gurobi).
...
Run: py mt_vgvb_py_v1_1.py
Outputs: results_mt_vgvb_py_v1_1/
"""
...
def config_default() -> Dict:
    ...
    return dict(CASE_NAME="ieee33", CASE_PARAM_MODE="enhanced33_paper", BASE_KV=12.66, BASE_MVA=1.0,
                ...
                ALLOW_DG_Q_SUPPORT=False, INCLUDE_RPC=True, INCLUDE_OLTC_FIXED=True,
                RPC_MODE="fixed_zero_assumption", OLTC_NOTE="OLTC fixed, not optimized in v1_1",
                ...
                n_random_checks=12, n_sensitivity_validation=8, random_seed=7,
...
def build_case_ieee33(config=None) -> Dict:
    ...
    # Table-I IEEE-33 loads in MW/MVAr. Bus 17 is 0.09 MW, making total P=3.715 MW, Q=2.300 MVAr.
...
def solve_omega_b_bound(bound_type,t,data,config):
    ...
    return dict(status=m.Status, objective=m.ObjVal, true_objective=float(expr.getValue()), runtime=rt, pB_agg=pa, EB_agg=np.cumsum(pa)*config['DT'], pCh=pChv, pDis=pDisv, eB=np.array([[e[b,i].X for i in range(T+1)] for b in range(nb)]), simultaneous_ch_dis_max=float(np.max(np.minimum(pChv,pDisv))))
...
def build_case_parameter_summary(case, resources, config):
    return [['CASE_PARAM_MODE',config['CASE_PARAM_MODE']],['BASE_KV',config['BASE_KV']],['BASE_MVA',config['BASE_MVA']],['Z_BASE',config['Z_BASE']],['total_P_load',case['total_P_load']],['total_Q_load',case['total_Q_load']],['DG buses/capacity', ';'.join(f"{r['bus']+1}:{r['pmax']}MW:q[{r['qmin']},{r['qmax']}]" for r in resources['G'])],['ESS buses/power/energy',';'.join(f"{r['bus']+1}:pch={r['pch']},pdis={r['pdis']},e=[{r['emin']},{r['emax']}],e0={r['e0']}" for r in resources['B'])],['ESS SOC assumption',config['ess_soc_assumption']],['RPC status',str(config['INCLUDE_RPC'])+' '+str(resources.get('RPC',[]))],['OLTC status',str(config['INCLUDE_OLTC_FIXED'])+'; '+config['OLTC_NOTE']],['line rating assumption',f"Smax={config['assumption_line_smax_mva']} MVA where Table IV is not provided"],['stage_i_pq_range_implemented',config['STAGE_I_PQ_RANGE_IMPLEMENTED']],['stage_ii_active_trajectory_implemented',config['STAGE_II_ACTIVE_TRAJECTORY_IMPLEMENTED']],['decoupling_note',config['decoupling_note']]]
...
def main():
    config=config_default(); out=Path('results_mt_vgvb_py_v1_1')
    ...
    case=build_case_ieee33(config); profiles=build_profiles(config['T'],config['DT']); resources=build_resources(config); base=solve_base_dispatch_gurobi(case,profiles,resources,config)
